...
class GantryFormFieldCtfonts extends GantryFormField
{
	...
	*/
	...
	protected $type = 'ctfonts';
	protected $basetype = 'ctfonts';
	...
	public function getInput()
	{
		...
		if (!self::$assets_loaded) {
			$gantry->addScript($gantry->gantryUrl . '/admin/widgets/fonts/js/fonts.js');
                ...
                jQuery('#jform_params_font3_family3').on('liszt:updated', function(e){
                    var thefunc = function(){
                        var thelist = jQuery('#jform_params_font2_family2').get(0);
                        for(var i = 0; i < thelist.options.length;i++){
                            if(thelist.options[i].value == '".($this->id == 'jform_params_font2_family2' ? $this->value : 'hadenukkenruppel')."'){
                    ...
                    thefunc = function(){
                        var thelist = jQuery('#jform_params_font1_family1').get(0);
                        for(var i = 0; i < thelist.options.length;i++){
                            if(thelist.options[i].value == '".($this->id == 'jform_params_font1_family1' ? $this->value : 'hadenukkenruppel')."'){
                                thelist.options[i].selected = true;
                                break;
                            }
                        }
